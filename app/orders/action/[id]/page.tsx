...
export default function UpdateOrderPage() {
  ...
  const fetchOrderById = async () => {
    const { data } = await axios.get<Order>(
      `https://main--steady-choux-73e324.netlify.app/.netlify/functions/api/orders/${params.id}`
    );
    fetchCars(data.car_id);
    form.setValue("car_id", data.car_id.toString());
    form.setValue("order_date", new Date(data.order_date));
    form.setValue("pickup_date", new Date(data.pickup_date));
    form.setValue("pickup_location", data.pickup_location);
    form.setValue("dropoff_date", new Date(data.dropoff_date));
    form.setValue("dropoff_location", data.dropoff_location);
  };
  useEffect(() => {
    fetchOrderById();
  }, [router]);

  const fetchCars = async (id: number) => {
    const { data } = await axios.get<Car[]>(
      "https://main--steady-choux-73e324.netlify.app/.netlify/functions/api/cars"
    );
    let availableCar = data.filter((car) => car.orders.length === 0 || car.car_id === id);
  ...
  const handleUpdateOrder = async (orderData: OrderWithoutId) => {
    try {
  ...
  return (
    <main>
      ...
      <div className="border rounded-lg p-8">
        <p className="text-center font-bold uppercase mb-4">Form Update Order</p>
        <Form {...form}>
